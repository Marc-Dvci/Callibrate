...
| `CBR_PUBLIC_VERIFICATION_DAILY_LIMIT` | `25` | Verifications anonymous visitors may cause in one UTC day on the pilot line, deployment-wide. |
...
- `POST /api/verify-now` starts one. A signed-in curator is always allowed. A
  visitor is allowed only while the caller is the pilot line, which dials
  nothing. A deployment with `CBR_CALLER_MODE=calle` answers a visitor with
  401, and there is no setting that reopens it: the five eligibility gates
  decide whether a number may be called, and a session decides who may spend
  that permission. On the pilot line, one address gets six requests a minute
  and the whole deployment gets `CBR_PUBLIC_VERIFICATION_DAILY_LIMIT` a day,
  counted in the process and reset at midnight UTC; the console's
  `/api/system` reports what is left.
